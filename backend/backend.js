...
const dbFilePath = config_1.config.dbPath;
...
app.post("/api/schedule", async (req, res) => {
    ...
    const db = await openDatabase();
    try {
        const result = await db.run("INSERT INTO schedule (date, hour, task) VALUES (?, ?, ?)", [date, hour, task]);
...
app.put("/api/schedule/:id", async (req, res) => {
    ...
    const db = await openDatabase();
    try {
        const result = await db.run("UPDATE schedule SET date = ?, hour = ?, task = ? WHERE id = ?", [date, hour, task, id]);

...
async function showCurrentTask() {
  ...
  const currentTask = await db.get(
    "SELECT * FROM schedule WHERE date = ? AND CAST(substr(hour, 1, 2) AS INTEGER) <= ? AND CAST(substr(hour, 4, 2) AS INTEGER) > ?",
    [today, currentHour, currentHour]
  );  

  // Query for the next task
  const nextTask = await db.get(
    "SELECT * FROM schedule WHERE date = ? AND CAST(substr(hour, 1, 2) AS INTEGER) > ? ORDER BY CAST(substr(hour, 1, 2) AS INTEGER) ASC LIMIT 1",
    [today, currentHour]
  );  
